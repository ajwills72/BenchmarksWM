...
Download the latest release from the 'releases' directory to your local machine and use `R CMD INSTALL BenchmarksWM_0.1.1.tar.gz` from the command line. Or from within R, navigate to the directory containing the downloaded file and use `install.packages("BenchmarksWM_0.1.1.tar.gz")`

## How to use
Once installed, type `library(BenchmarksWM)`. For an overview help file including a list of datasets, type `?BenchmarksWM`. For documentation on a particular dataset, type e.g. `?adam15`. To view a dataset, type e.g. `View(adam15)`
